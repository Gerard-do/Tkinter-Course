...
def submit_name():
    ...
    #Create a label for the username based for radio button values
    if case_style.get() == 'normal':
        name_label = tkinter.Label(output_frame, text="Hello "+name.get() + '!! I keep learning Tkinter!!', bg=output_color)
    elif case_style.get() == 'upper':
        name_label = tkinter.Label(output_frame, text=("Hello "+name.get() + '!! I keep learning Tkinter!!').upper(), bg=output_color)
...
#Create Widgets
name = tkinter.Entry(input_frame, text='Enetr your name', width=20)
...
normal_button.grid(row=1, column=0, pady=2)
upper_button.grid(row=1, column=1, pady=2)

#Add an image code
#smile_img = ImageTk.PhotoImage(Image.open('name of file'))
#smile_label = tkinter.Label(output_frame, image='smile_img, bg=outout_color)
#smile_label.pack()
